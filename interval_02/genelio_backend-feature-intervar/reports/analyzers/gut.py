...
# pdfplumber is imported lazily inside extract_all_tables so the Django app
# can boot in environments where the gut-microbiome pipeline deps aren't
# installed yet. See requirements.txt for the full list.

# Re-exported from the shared module so existing call sites (including the
# legacy e2e reports and the RAG pipeline) keep working without edits.
...
def extract_all_tables(pdf_path: str) -> dict:
    ...
    import pdfplumber  # lazy: only needed when a gut report is actually parsed
    report = {
        "patient": {},
        "keystone_present": [],
        "keystone_missing": [],
